...
namespace IsObservableCollBuggy.Droid.Effects
{
    public class BasePlatformEffect : PlatformEffect
    {
        ...
        void BindingContextChanged(object sender, EventArgs e)
        {
            ...
            if (IsAttached)
            {
                var toRemove = Element.Effects.OfType<BaseRoutingEffect>().FirstOrDefault(x => x.EffectId == ResolveId);
        ...
        static void IsRegisteredPropertyChanged(BindableObject bindable, object oldValue, object newValue)
        {
            ...
            void RemoveEffects()
            {
                ...
                {
                    foreach (var effect in child.Effects.OfType<BaseRoutingEffect>())
                    {
                        Device.BeginInvokeOnMainThread(() => child.Effects.Remove(effect));
                    }
                }

                foreach (var effect in page.Effects.OfType<BaseRoutingEffect>())
                {
